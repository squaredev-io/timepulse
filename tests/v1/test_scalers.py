...
def test_scalers():
    ...
    assert np.allclose(transformed_X, np.array([[0.0, 0.0], [0.5, 0.5], [1.0, 1.0]]))
    ...
    assert np.allclose(transformed_y, np.array([[0.0], [0.5], [1.0]]))
    ...
    transformed_y = scaler_wrapper.fit_transform_y(data_y.reshape(-1, 1))
    assert np.allclose(
        transformed_y.flatten(), expected_transformed_y, rtol=1e-5, atol=1e-8
    )
