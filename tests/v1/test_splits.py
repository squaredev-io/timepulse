...
def test_splits():
    ...
    train_windows, test_windows, train_labels, test_labels = make_window_splits(
        values, size=window_size, horizon=horizon
    )

    assert (
        len(train_windows) + len(test_windows)
        == len(values) - (window_size + horizon) + 1
    )
    assert (
