...
def test_xgboost():
    ...
    ), "Number of training samples in X_train and y_train do not match"
    assert (
        X_test.shape[0] == y_test.shape[0]
    ), "Number of testing samples in X_test and y_test do not match"
    ...
    y_pred, result_metrics = run_model(
        model_instance, X_train, y_train, X_test, y_test, verbose=0
    )
    ...
    y_pred, result_metrics = run_model(
        default_model_instance, X_train, y_train, X_test, y_test, verbose=0
    )
    assert y_pred.shape == y_test.shape, "Shape mismatch between y_pred and y_test"
    ...
    with patch("joblib.dump") as mock_dump:
        ...
        mock_dump.assert_called_once_with(
            default_model_instance.model, expected_filename
        )
